...
static CONTROL_SCHED_t sched_allowed[][SCHED_ALLOWED_WIDTH] = {
		...
		{CONTROL_SCHED_ABORT, CONTROL_SCHED_SHUTDOWN, CONTROL_SCHED_NOTHING, CONTROL_SCHED_NOTHING, CONTROL_SCHED_NOTHING, CONTROL_SCHED_NOTHING}, 	//THRUST
...
static void control_update(CONTROL_INST_t * control) {
	...
	if(control->target_counter_active) {
	...
	while(can_msgPending()) {
		...
		if(control->msg.id == DATA_ID_THRUST_CMD) {
			if(control->msg.data == 0) {
				control_shutdown();
			}
		}
	}
...
static void calibration(CONTROL_INST_t * control) {
	...
	if(control->pp_hom_started) {
		control->pp_hom_started++;
		if(control->pp_hom_started > TARGET_REACHED_DELAY_CYCLES) {
...
static void init_thrust(CONTROL_INST_t * control) {
	...
	control->target_counter = control->pp_params.target_wait;
...
static void thrust(CONTROL_INST_t * control) {
	...
	if(control->safety_counter <= TIME_TOL) {
		...
		//comment if shutdown should happen after command arrived too soon
		if(control_sched_should_run(control, CONTROL_SCHED_SHUTDOWN)) {
			control->safety_triggered = 1;
			control_sched_done(control, CONTROL_SCHED_SHUTDOWN);
		}
	}
